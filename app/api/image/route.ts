    ...
    if (!referenceImage) {
      ...
      if (isVariation) {
        const variationText = variationIndex === 0
          ...
          : `This is the original ad creative. Create Variation 2 — Before/After Split format (${ratioLabel}). Divide the image into two halves with a clean subtle split line or natural contrast. LEFT HALF — Before (the problem/pain state): dark, moody, raw — show the struggle, frustration, or undesirable situation the target audience relates to, muted tones, heavy shadows, tense body language or visual metaphor for the problem. RIGHT HALF — After (the result/transformation): bright, clean, aspirational — show the outcome, confidence, relief, success, or the desirable end state, warm or vibrant tones, open lighting, positive energy. HEADLINE: left side shows pain/problem statement in dark tone medium weight, right side shows result/solution statement in primary brand color bold, both pulled from the original ad's angle and message, all colors and fonts follow the brand reference from the original. Keep the same brand, product, and subject from the original. Logo centered at the bottom or bottom right, medium size with subtle glow. Final output must look like a ready-to-run Facebook link ad.`;
