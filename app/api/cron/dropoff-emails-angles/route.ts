...
const EMAIL_TYPE = "angles_to_copy_nudge";
...
function getSubject(firstName?: string) {
  ...
  return `${name}, you found your angle. Time to write the copy.`;
}
...
async function fetchLatestAnglesTransactions(admin: ReturnType<typeof adminClient>) {
  const rows = await fetchPagedRows<CreditTransactionRow>(async (from, to) => {
    const { data, error } = await admin
      .from("credit_transactions")
      .select("user_id, created_at")
      .eq("type", "use")
      .eq("description", "Angles generation")
...
export async function GET(req: NextRequest) {
  ...
  if (!acquireCronLock("dropoff-emails-angles")) {
  ...
  try {
    const [latestAnglesByUser, copyUserIds, emailedUserIds] = await Promise.all([
      fetchLatestAnglesTransactions(admin),
      fetchUserIdSetByFilter(admin, "credit_transactions"),
      fetchUserIdSetByFilter(admin, "email_log"),
    ]);

    const candidateUserIds = Array.from(latestAnglesByUser.entries())
    ...
    for (const userId of candidateUserIds) {
      ...
      try {
        ...
        console.error("[cron-dropoff-emails-angles] user processing error:", error);
    ...
    console.error("[cron-dropoff-emails-angles] cron error:", error);
