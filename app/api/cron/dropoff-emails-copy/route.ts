...
const EMAIL_TYPE = "copy_to_launch_nudge";
...
function getSubject(firstName?: string) {
  ...
  return `${name}, your campaign assets are done. One step left.`;
}
  ...
  return `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 560px; margin: 0 auto; padding: 24px; color: #1F2937;">
      ...
      <p style="margin: 32px 0;">
        <a href="${APP_URL}/campaign-setup" style="display: inline-block; background: #EF4444; color: #FFFFFF; font-weight: bold; padding: 14px 28px; border-radius: 8px; text-decoration: none; font-size: 14px;">Launch My Campaign &rarr;</a>
      </p>
      ...
      <p style="font-size: 12px; color: #9CA3AF; line-height: 1.5;">You're receiving this because you signed up for Hinilas Pro and haven't launched your campaign yet. <a href="${APP_URL}/home" style="color: #9CA3AF;">Visit your dashboard</a>.</p>
    </div>
  `;
}
...
async function fetchLatestCopyTransactions(admin: ReturnType<typeof adminClient>) {
  const rows = await fetchPagedRows<CreditTransactionRow>(async (from, to) => {
    const { data, error } = await admin
      .from("credit_transactions")
      .select("user_id, created_at")
      .eq("type", "use")
      .eq("description", "Copy generation")
...
async function fetchUserIdSetByFilter(
  ...
  if (table === "credit_transactions") {
    const rows = await fetchPagedRows<CreditTransactionRow>(async (from, to) => {
      ...
      if (error) throw new Error(`Failed to fetch campaign transactions: ${error.message}`);
      return data || [];
    });
...
export async function GET(req: NextRequest) {
  ...
  if (!acquireCronLock("dropoff-emails-copy")) {
  ...
  try {
    const [latestCopyByUser, campaignUserIds, emailedUserIds] = await Promise.all([
      fetchLatestCopyTransactions(admin),
      fetchUserIdSetByFilter(admin, "credit_transactions"),
      fetchUserIdSetByFilter(admin, "email_log"),
    ]);

    const candidateUserIds = Array.from(latestCopyByUser.entries())
      .filter(([, createdAt]) => createdAt <= cutoffIso)
      .map(([userId]) => userId)
      .filter((userId) => !campaignUserIds.has(userId) && !emailedUserIds.has(userId));
    ...
    for (const userId of candidateUserIds) {
      ...
      try {
        ...
        console.error("[cron-dropoff-emails-copy] user processing error:", error);
    ...
    console.error("[cron-dropoff-emails-copy] cron error:", error);
