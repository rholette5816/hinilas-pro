...
Translate this angle into a visual. The image must communicate the core message and emotion of this angle — the visual itself should tell the story. A viewer who does not read the headline should still feel the emotion from the image alone.

FORMAT: ${format} aspect ratio
${logoDesc ? `BRAND REFERENCE: ${logoDesc}\n- Extract and follow exactly: brand colors, font style, font weight, graphic style, logo placement, and overall visual identity` : ""}
${productDesc ? `HERO SUBJECT: ${productDesc}\n- Feature this subject as the focal point of the ad` : ""}
${extraDetails ? `CREATIVE DIRECTION: ${extraDetails}` : ""}

GRAPHIC & TYPOGRAPHY TREATMENT:
- Text must be designed, not just placed — use highlight bars, color blocks, or graphic backing behind headlines
- Typography is part of the visual composition — oversized, layered, or integrated into the scene
- Add infographic elements relevant to the angle: stat badges, result callouts, before/after labels, icon accents
- Use graphic shapes to frame the layout — lines, geometric forms, swooshes, or overlays that follow the brand style
- The overall look must feel designed by a professional agency, not generated by AI

DESIGN RULES:
- Headline: 3–6 words max, bold, emotionally charged — pulled directly from the angle's hook or core message
- ALL TEXT spelled correctly — zero typos, zero distortion
- Hero subject is the focal point — sharp, dramatic lighting
- Composition: headline top or center, subject prominent, brand mark bottom
- One clear emotional message — no clutter
- Color mood must match the angle's emotion: warm/orange/red for urgency and pain, cool/blue for trust and clarity, gold for premium transformation
${logoDesc ? `- Colors, fonts, and graphic style must follow the brand reference above` : `- Default palette: dark navy / near-black background, white headline, bright orange accent`}
